...
class _Spa {
  ...
  _Spa clone() {
    ...
    c.slope = slope;
    c.azmRotation = azmRotation;
    c.atmosRefract = atmosRefract;
    c.function = function;
    c.jd = jd;
    c.jc = jc;
    c.jde = jde;
    ...
    c.xi = xi;
    c.delAlpha = delAlpha;
    c.deltaPrime = deltaPrime;
    c.alphaPrime = alphaPrime;
    c.hPrime = hPrime;
    c.e0 = e0;
    c.delE = delE;
    c.e = e;
    ...
    c.sta = sta;
    c.zenith = zenith;
    c.azimuthAstro = azimuthAstro;
    c.azimuth = azimuth;
    c.incidence = incidence;
    c.suntransit = suntransit;
    c.sunrise = sunrise;
    c.sunset = sunset;
    return c;
  }
...
double _geocentricRightAscension(double lamda, double epsilon, double beta) {
  ...
  return _limitDegrees(
    _rad2deg(
      atan2(
        sin(lambdaRad) * cos(epsilonRad) -
            tan(_deg2rad(beta)) * sin(epsilonRad),
        cos(lambdaRad),
      ),
    ),
  );
...
double _julianDay(
  ...
  double jd =
      (365.25 * (y + 4716.0)).floor().toDouble() +
...
double _meanElongationMoonSun(double jce) => _thirdOrderPolynomial(
  1.0 / 189474.0,
  -0.0019142,
  445267.11148,
  297.85036,
...
double _sunHourAngleAtRiseSet(
  ...
  final argument =
      (sin(_deg2rad(h0Prime)) - sin(latRad) * sin(deltaZeroRad)) /
